...
TRAITE_GRAPHIQUE = (file) => {
  console.log(file)
  Promise.all([
    EXTRACT_CORRELATION(file.correlations),
    EXTRACT_DBMETADATA(file.dbMetadata)
  ])
  .then(
    result => {
      console.log(result)
...
TRAITE_DATA = (ArrFichiers) => {
  for (var i = 0; i < ArrFichiers.length; i++) {
    if (TEST_FILES(ArrFichiers[i].$.href) === 'JPEG') {
      TRAITE_GRAPHIQUE(ArrFichiers[i])
    } else {
      TRAITE_DOCUMENT(ArrFichiers[i])
    }
  }
}
...
EXTRACT_DATA = (data) => {
  EXTRACT_FILES(data.prodexport.file)
    ...
    .then(result => TRAITE_DATA(result))
}
